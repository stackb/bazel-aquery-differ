...
Copied from https://pkg.go.dev/github.com/bazelbuild/bazel-gazelle/testtools
*/
...
`, updateCommand)
		...
		if config.ExitCode != actualExitCode {
			errs = append(errs, fmt.Errorf("expected aquerydiff exit code: %d\ngot: %d",
		...
		if strings.TrimSpace(config.Stdout) != strings.TrimSpace(actualStdout) {
			errs = append(errs, fmt.Errorf("expected aquerydiff stdout: %s\ngot: %s",
		...
		if strings.TrimSpace(config.Stderr) != strings.TrimSpace(actualStderr) {
			errs = append(errs, fmt.Errorf("expected aquerydiff stderr: %s\ngot: %s",
